    ...
    <div class="row">
      <h1 class="ui header">
        <?php
          if($pr_flag){
            echo "$id: ".$row['title'];
            // <%= problem.title %><% if (problem.allowedEdit && !problem.is_public) { %><span class="ui tiny red label">미공개</span><% } %>";
          ...
          if($row['defunct']=="Y")
          echo "<span class=\"ui tiny red label\">미공개</span>";
        ?>
      </h1>
    </div>
      <div class="row" style="margin-top: -15px">
          <span class="ui label">제한메모리:<?php echo $row['memory_limit']; ?> MB</span>
          <span class="ui label">시간제한：<?php echo $row['time_limit']; ?> S</span>
          <!-- <span class="ui label">제목유형：交互</span> -->
          ...
          <span class="ui label">표준입출력</span>
      </div>
      <div class="row" style="margin-top: -23px">
          <span class="ui label">제목유형：전통</span>
          <span class="ui label">평가방식:<?php if($row['spj']) echo "Special Judge"; else echo "텍스트 비교" ; ?></span>
          <span class="ui label">게시자：<span id='creator'></span></span>
      </div>
      <div class="row" style="margin-top: -23px">
...
<div class="ui grid">
  <div class="row"> 
    <div class="column">
      <div class="ui buttons">
          ...
          <?php
            if($pr_flag){
              echo "<a  id='submit'  class=\"small ui primary button\" href=\"submitpage.php?id=$id\">제출</a>";
              echo "<a class=\"small ui positive button\" href=\"status.php?problem_id=$id\">제출기록</a>";
	      ...
	      echo "<a class=\"small ui red button\" href=\"discuss.php?pid=$id\">$MSG_BBS</a>";
              ...
              echo "<a  id='submit'  href=\"contest.php?cid=$cid\" class=\"ui orange button\">재도전</a>";
              echo "<a class=\"small ui primary button\" href=\"submitpage.php?cid=$cid&pid=$pid&langmask=$langmask\">제출</a>";
              echo "<a class=\"small ui positive button\" href=\"status.php?problem_id=$PID[$pid]&cid=$cid\">제출기록</a>";
            }
          ?>
  ...
  <div class="row">
    <div class="column">
      <h4 class="ui top attached block header">제목설명</h4>
  ...
  <?php if($row['input']){ ?>
    <div class="row">
      <div class="column">
          <h4 class="ui top attached block header">입력 양식</h4>
  ...
  <?php if($row['output']){ ?>
    <div class="row">
        <div class="column">
          <h4 class="ui top attached block header">출력 양식</h4>
  ...
  <?php if(strlen($sinput)){ ?>
    <div class="row">
        <div class="column">
          ...
          <span class="copy" id="copyin" data-clipboard-text="<?php echo ($sinput); ?>">복제</span>
          </h4>
          <!-- <span class=copy id=\"copyin\" data-clipboard-text=\"".($sinput)."\">복제</span> -->
  ...
  <?php if(strlen($sinput)){ ?>
    <div class="row">
        <div class="column">
          ...
          <span class="copy" id="copyout" data-clipboard-text="<?php echo ($soutput); ?>">복제</span>
          </h4>
          <!-- <span class=copy id=\"copyout\" data-clipboard-text=\"".($soutput)."\">복제</span> -->
          <div class="ui bottom attached segment font-content">
  ...
  <?php if($row['hint']){ ?>
    <div class="row">
        <div class="column">
          <h4 class="ui top attached block header">데이터 출처</h4>
...
<?php include("template/$OJ_TEMPLATE/footer.php");?>
  ...
  <script>
    ...
    clipboardin.on('success', function(e){
      $("#copyin").text("복제성공!"); 
          setTimeout(function () {$("#copyin").text("복제"); }, 1500);    
      console.log(e);
    });
    ...
    clipboardout.on('success', function(e){
      $("#copyout").text("복제성공!"); 
          setTimeout(function () {$("#copyout").text("복제"); }, 1500);    
      console.log(e);
    });
